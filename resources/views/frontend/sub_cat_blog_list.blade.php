...
@section('schema')
    <script type='application/ld+json'>
    {
        ...
        "dateModified": "{{ $subCategory->updated_at ?? $subCategory->created_at }}",
        "publisher": {
        "@type": "Organization",
        "name": "CodingIz",
        "logo": {
    ...
    <div class="breadcrumbs panel z-1 py-2 bg-gray-25 dark:bg-gray-100 dark:bg-opacity-5 dark:text-white">
        <div class="container max-w-xl">
            <ul class="breadcrumb nav-x justify-center gap-1 fs-7 sm:fs-6 m-0">
                ...
                <li><i class="unicon-chevron-right opacity-50"></i></li>
                <li><a href="{{ route('sub.cat.post.list',['category' => $category->slug,'sub_category' => $subCategory->slug]) }}">{{ $subCategory->name }}</a></li>
            </ul>
        </div>
    </div>
        ...
        <div class="container max-w-xl">
            <div class="panel vstack gap-3 sm:gap-6 lg:gap-9">
                <header class="page-header vstack justify-center items-center text-center max-w-500px mx-auto">
                    <h1 class="h4 lg:h1">{{ $subCategory->name }}</h1>
                    <p class="fs-6 lg:fs-5 opacity-60">{{ $subCategory->meta_description }}</p>
                </header>
